...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	<xsl:param name="welcomeMsg">
		<div><p class="title main">Welcome to the Digital Vercelli Book!</p></div>

		<div>This is a facsimile and diplomatic edition of Codex Vercellensis CXVII, Archivio e Biblioteca Capitolare di Vercelli. 
			Here are a few tips about how to better browse the edition:
        </div>
		<div>
			<ul>
				<li>it is recommended to go to full screen mode so that all available screen space is used to show the manuscript images and the transcription text;</li>
				<li>speaking of "text", note that unfortunately only part of the homilies and poems are currently available, they will be all present in the final version of the edition;</li>
				<li>note that the digitized images, while not too heavy, require some time to load in the browser window, so we advise against going too fast when moving from folio to folio, lest the navigation software gets confused;</li>
				<li>finally, if you have any suggestions or spot an error/bug please contact me at 
					<a href="mailto:roberto.rossellidelturco@unito.it">roberto.rossellidelturco@unito.it</a>.</li>
			</ul>
		</div>
		<div>This edition was created using <a href="http://evt.labcd.unipi.it/" target="blank">EVT</a>.</div>        
	</xsl:param>
	...
	<!-- default: true() -->
	<xsl:param name="use_list_head_as_label" select="true()"/>
